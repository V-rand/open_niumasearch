...
class RunLogger:
    ...
    def _trace_model_request(self, payload: dict[str, Any]) -> list[str]:
        ...
        lines.append("**模型输入请求 (Request Snapshot)**")
        if message_count is not None:
            lines.append(f"- 当前消息总数: `{message_count}`")
        if isinstance(tool_names, list) and tool_names:
            lines.append(f"- 可用工具: {', '.join(f'`{n}`' for n in tool_names)}")
        ...
        if isinstance(messages, list):
            ...
            lines.append("\n<details>")
            lines.append(f"<summary>查看本次请求的完整上下文 (Full Message Stack - Turn {turn_index})</summary>\n")
            for i, msg in enumerate(messages):
                role = msg.get("role", "unknown")
                # Optimization: Hide system prompt content after turn 1
                if role == "system" and turn_index > 1:
                    lines.append(f"#### SYSTEM")
                    lines.append("> (System Prompt content omitted; refer to Turn 1 or system_prompt.txt)")
                    lines.append("\n---\n")
                    continue
                    
                content = msg.get("content") or ""
                lines.append(f"#### {role.upper()}")
                lines.append(self._compact_preview(str(content), max_lines=50))
